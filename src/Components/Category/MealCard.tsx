...
const MealCard:React.FC <{data: Meal, category: string}> = ({data, category}) => {
    ...
    <img onMouseEnter={mouseEnterHandler}  src={data.strMealThumb} alt="" />
    {hover && <><div onClick={clickHandler} onMouseLeave={mouseEnterHandler} className="title"><h1>{data.strMeal}</h1></div></>}
    </CardContainer>
}
